...
class TimeManageTool
    ...
    @main_array.each_with_index do |collumn, index_y|
      collumn.each_with_index do |item, index_x|
        ...
        if @args.inputs.mouse.click 
          ...
          elsif mouse_click(@args.inputs.mouse.point, task_grid_traits) && @main_array[index_y][index_x] == nil
            @clicked_subtask = @main_array[index_y][0]
  ...
  def new_task_screen()
    ...
    @new_task = [ ["TASK", "-"], ["OBJECTIVE", "-"], ["SUBOBJECTIVE?", "-" ]]
    @new_subtasks = []

    @new_task.each_with_index do |new_task|
      if @clicked_subtask == new_task[0]
        puts @clicked_subtask
        new_task[1] = new_task[0]
      end
    end
