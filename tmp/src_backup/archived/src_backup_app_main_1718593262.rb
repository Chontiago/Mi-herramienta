...
class TimeManageTool
  ...
  def main_buttons()
    ...
    @main_buttons.each_with_index do |button, index_x|
      ...
      elsif mouse_click(@args.inputs.mouse.point, create_task_button ) && @create_task_text[:text] == "CREATE" #Creates the new task and gets back to the task and subtask screen when this button is pressed
      @create = true
       
      elsif mouse_click(@args.inputs.mouse.point, create_task_button) && @create_task_text[:text] == "BACK"
      $gtk.reset
      end
    end
  end
