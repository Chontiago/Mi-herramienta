    ...
    @main_array.each_with_index do |collumn, index_y|
      collumn.each_with_index do |item, index_x|
        ...
        if @args.inputs.mouse.click
          ...
          elsif mouse_click(@args.inputs.mouse.point, create_task_button ) && @new_task_buttons[0] == "CREATE" #Creates the new task and gets back to the task and subtask screen when this button is pressed
            @create = true
               
          elsif mouse_click(@args.inputs.mouse.point, create_task_button) && @main_button_text == "BACK"
            $gtk.reset
          end
        end
      end
