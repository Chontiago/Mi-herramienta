...
class TimeManageTool
  ...
  def new_task_grid()
    @new_task.each_with_index do |new_task, index_y|
      new_task.each_with_index do |element, index_x|
        @new_task_grid_x = (index_x * TILE_WIDTH + HALF_SCREEN_X)
        @new_task_grid_y = (index_y * TILE_HEIGHT + HALF_SCREEN_Y)

        task_grid_traits = {x: @new_task_grid_x, y: @new_task_grid_y, w: TILE_WIDTH, h: TILE_HEIGHT}
  ...
  def new_task_behaviour()
    if @start_typing == true
      ...
      @args.outputs.labels << {x: @new_task_grid_x, y: @new_task_grid_y, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{@word}"}
