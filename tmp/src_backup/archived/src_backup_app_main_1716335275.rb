...
class TimeManageTool
  ...
  def new_task_screen()
    counter ||= 0
    @new_task_options = [["TASK", nil],["Subtasks?", nil], ["Objective", nil]]
    @new_task ||= @new_task_options[0]
    ...
    @selected_option_full = []
    @start_typing = false
    @new_task_options.each_with_index do |new_task, index|
        @new_task = [@new_task_options[index]]
    end
