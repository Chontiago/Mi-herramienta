...
class TimeManageTool
  ...
  def render()
    ...
    if @clicked_new_task == false #Makes it so objective colors are only present when task or subtasks are present
    ...
    @loaded_data.each do |hash| #The main content (The tasks and subtasks) is loaded from the save_data.txt
      hash.each_pair do |key, value|
        @task_array.append(key)
          ...
          if @clicked_new_task == true #If NEW TASK button is pressed displays the options to create a new task
            @main_array = @new_task
          end
      end
    end
    ...
    @main_array.each_with_index do |collumn, index_y|
      collumn.each_with_index do |item, index_x|
        ...
        if @args.inputs.mouse.click
          ...
          elsif mouse_click(@args.inputs.mouse.point, @create_task_button) && @new_task_buttons[0] == "NEW TASK"
            @clicked_new_task = true
