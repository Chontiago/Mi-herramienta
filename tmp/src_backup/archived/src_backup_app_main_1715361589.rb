...
class TimeManageTool
  ...
  def create_task_button
    @main_array.each_with_index do |collumn, index_y|
      collumn.each_with_index do |item, index_x|
        ...
        if @args.inputs.mouse.click 
          if mouse_click(@args.inputs.mouse.point, create_task_button) 
            @args.outputs.labels << {x: 1280 / 2, y: 100, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"Ahhhh"}
            @new_task_screen = true
  ...
  def new_task_screen()
    @new_task = [{"TASK" => nil}, {"OBJECTIVE" => nil}, {"HAS SUBTASK?" => nil}]
    @new_subtasks = []
    @current_screen ||= @new_task
  ...
  def render_new_task_screen()
    ...
    @current_screen.each_with_index do |item, index_y|
      item.each_pair_with_index do |key, value, index_x|
      ...
      #grid_x = (index_x * TILE_WIDTH) + HALF_SCREEN_X
        ...
        task_grid_items_traits = {x: HALF_SCREEN_X + HALF_TILE_WIDTH, y: grid_y + HALF_TILE_HEIGHT, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{key}"}
