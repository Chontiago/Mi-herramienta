...
class TimeManageTool
  ...
  def time_reset # Resets task objectives every monday
    ...
    elsif day_of_the_month > current_day + 7
  ...
  def create_task_button
    @main_array.each_with_index do |collumn, index_y|
      collumn.each_with_index do |item, index_x|
        grid_x = (index_x - 3)  + HALF_SCREEN_X
        @new_task_grid_y = (index_y - 10) - TILE_HEIGHT + HALF_SCREEN_Y
        @create_task_button = {x:grid_x , y: @new_task_grid_y, w: TILE_WIDTH, h: TILE_HEIGHT, r: 0, g:0, b: 255}
  ...
  def new_task_behaviour()
    if @start_typing == true
      ...
      @args.outputs.labels << {x: 1280 / 2, y: @new_task_grid_y, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{@word}"}
