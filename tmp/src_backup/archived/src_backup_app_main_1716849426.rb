...
class TimeManageTool
  ...
  def create_button
    @create ||= false
    if @object.length >= 3
      @create_task_text[:text] = "CREATE"
    end

    if @args.inputs.mouse.click
      if mouse_click(@args.inputs.mouse_point, @create_task_button) && @create_task_text == "CREATE"
        @args.outputs.labels << {x: 1280 / 2, y: 100, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"ahhhhhh"}
      end
    end
  end
