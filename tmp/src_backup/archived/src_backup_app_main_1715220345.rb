...
class TimeManageTool
  ...
  def time_reset
    ...
    time = Time.new
    weekday = time.wday
    day_of_the_month = Time.now.day
    #if weekday == 3
      #@args.gtk.write_file(MONDAY_FILE, "#{day_of_the_month}")
    #end
    ...
    if day_of_the_month > current_day + 6 && weekday == 1
      @args.gtk.write_file(SAVE_DATA, "#{@reset_tasks}")
      @args.gtk.write_file(MONDAY_FILE, "#{(day_of_the_month)}") 
    elsif day_of_the_month < current_day && weekday == 1
      @args.gtk.write_file(SAVE_DATA, "#{@reset_tasks}")
      @args.gtk.write_file(MONDAY_FILE, "#{(day_of_the_month)}") 
    ...
    #monday = eval(@args.gtk.read_file(MONDAY_FILE))
    ...
    #@args.gtk.write_file(MONDAY_FILE, "#{counter}")
   

    #if weekday == 2  && @args.state.tick_count == 1 
      #@args.gtk.write_file(SAVE_DATA, "#{@reset_tasks}")
     # @monday = true
