...
class TimeManageTool
  ...
  def new_subtasks_and_subobjectives
    @new_subtasks ||= []
    @new_subobjectives = []
    @new_subtasks_and_subobjectives = {}

    if @args.inputs.keyboard.key_down.enter && @counter > 3
      @new_subtasks.append(@word)
    end
    @args.outputs.labels << {x: 1280 / 2, y: 50, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{@new_subtasks}"}
    puts @new_subtasks

    @new_subtasks.each_with_index do |subtasks_and_subobjectives, index|
      if index % 2 == 0
        @new_subtasks_and_subobjectives[subtask] = @new_subtasks[index + 1] if index + 1 < @new_subtasks.size
      end
    end
    #@args.outputs.labels << {x: 1280 / 2, y: 50, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{@new_subtasks_and_subobjectives}"} 
  end
    ...
    if @create == true
      if @object[2] != nil # Makes sure every element of the object is typed before creating the object
       ...
       if @limit == 2 # Checks if the object has subtasks or subtasks and creates the object accordingly
          ...
          new_object = TasksAndSubTasks.new(@object[0], ['PARTIDAS', 'FUNDIES', 'OTRAS'], [@object[2].to_i, @object[2].to_i, @object[2].to_i], @object[1]).generated_task_and_subtask_hash
        end
