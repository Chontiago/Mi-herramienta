...
class TimeManageTool
  ...
  def main_buttons() #Functionality for the buttons outside of the array that control the navigation between the  different arrays and features of the tool 
    ...
    @main_buttons.each_with_index do |button, index_x|
      ...
      #@clicked_subtask = nil
      if @args.inputs.mouse.click
        if mouse_click(@args.inputs.mouse.point, @create_task_button) && @main_button_text == "NEW TASK" #Changes from the main task and subtask screen to the new task creator
  ...
  def add_or_substract_to_objective() # When th eplus or substract button is pressed on the task or the subtask, adds or subtracts to the objective/sub objective
  ...
  def render_main_buttons
    ...
    @initial_tasks.each_pair do |hash|
      hash.each_pair do |task, subtask|
        if @clicked_new_task == "New Task" || @main_array == subtask
        @main_buttons = ["BACK"]
        end
      end
    end
