...
=begin
 COSAS A AGREGAR EN VERSIÓN 2.0

Reemplazar funcionalidad de botón + por botón de undo
Header con explicación de que hace cada columna
Mejoras a la UI de Create task
  -Instrucciones de que poner en cada pantalla
  -limitar las respuestas a yes or no en ciertas partes y a permanent o temporary
  -Que el botón de create task solo aparezca si tienes ya los diccionarios con sub objectives completos
  -Que se vayan agregando columnas con las subtasks y sub objectives en lugar de reiniciarse la pantalla 
  -Poder borrar letras si tienes un typo
Hacer que la tabla baje una tile cada vez que se crea una nueva task
Calendario con mis eventos guardados de que he hecho cada día
Función plara planear actividades del día
Hacer que se auto actualizen actividades de la semana si se actualizan actividades del día
Compatibilidad con siguientes herramientas
...
class TimeManageTool
  ...
  class  TasksAndSubTasks
    def initialize(task, subtasks, sub_objective, has_subtasks, permanent)
      @subtasks = subtasks
      @plus = '+'
      @minus = '-'
    ...
    def generate_sub_task
      ...
      @subtasks.each_with_index do |subtask, index|
        subtask_menu.append( [ subtask, @sub_objective[index],  @minus, @plus ] )
      end
  ...
  def initial_tasks() #Creates new tasks with their respective subtasks as objects to populate the main array
    SAVE_DATA = "save_data.txt" #Saves the changes in objectives during the week and temporary tasks
    MONDAY_RESET_DATA = "reset_data.txt" #Saves the initial state of the tasks plus the new added tasks for the reset intended every monday
    @programar ||= TasksAndSubTasks.new('PROGRAMAR',[], [5], "NO", true).generated_task_and_subtask_hash

    @smash ||= TasksAndSubTasks.new('SMASH', ['PARTIDAS', 'FUNDIES', 'OTRAS'], [2, 2, 1], "YES", true).generated_task_and_subtask_hash

    @caminar ||= TasksAndSubTasks.new('CAMINAR',['SOLO', 'PERRO'],[3, 2], "YES", true ).generated_task_and_subtask_hash

    @leer ||= TasksAndSubTasks.new('LEER', [], [3], "NO", true).generated_task_and_subtask_hash

    @initial_tasks ||= [ @programar, @caminar, @leer ] #The array that will be saved and modified in the SAVE DATA FILE

    @reset_tasks ||= [ @programar, @caminar, @leer ] #The array that will be saved in MONDAY_RESET_DATA to be reset every monday
  ...
  def load_data
    ...
    @reset_data = eval(@args.gtk.read_file(MONDAY_RESET_DATA))  

    if @args.state.tick_count == 1
      @initial_tasks = @loaded_data
      @reset_tasks = @reset_data
    end
  end
  ...
  def main_array() #Function determines what content goes on the main array to be displayed in the render function
    ...
    @back ||= false #flag to change the main button to back when main array is displaying subtasks
  ...
  def plus_and_minus_buttons() #If task has no subtasks, adds or substracts from objective, if it has subtasks takes you to subtask menu to add them individually
    @create = false
    @clicked_task = nil
    @clicked_subtask ||= nil
      ...
      collumn.each_with_index do |item, index_x|
        ...
        if @args.inputs.mouse.click
          if mouse_click(@args.inputs.mouse.point, task_grid_traits) && @main_array[index_y][index_x] == '-'
            ...
            if @main_array == @task_array #Copies the title of the task, and compares it to the keys initia_tasks_array to see if it has subtasks, and if it has, the substract function changes the main array to the subtasks array
              @clicked_subtask = @main_array[index_y][0]
            end

          elsif mouse_click(@args.inputs.mouse.point, task_grid_traits) && @main_array[index_y][index_x] == '+' #To be replaced with undo button in version 2.0
  ...
  def main_buttons() #Functionality for the buttons outside of the array that control the navigation between the  different arrays and features of the tool 

    @main_buttons.each_with_index do |button, index_x|
      ...
      #if mouse_click(@args.inputs.mouse.point, task_grid_traits) && @main_array[index_y][index_x] == 'BACK' #Gets back to the main grid if button is
      #@clicked_subtask = nil
      if @args.inputs.mouse.click
        if mouse_click(@args.inputs.mouse.point, @create_task_button) && button == "NEW TASK" #Changes from the main task and subtask screen to the new task creator
        @clicked_new_task = "New Task"
        @back = true
    
        elsif mouse_click(@args.inputs.mouse.point, create_task_button ) && button == "CREATE" #Creates the new task and gets back to the task and subtask screen when this button is pressed
        @create = true
       
        elsif mouse_click(@args.inputs.mouse.point, create_task_button) && button == "BACK" #Resets the array, returns to the main tasks
        $gtk.reset
        end
      end
    end
  ...
  def substract()
    @initial_tasks.each do |hash|
      hash.each_pair do |tasks, subtasks|
        ...
        elsif @clicked_task == tasks[0] + "+" && subtasks == []
          tasks[1] +=1
          @overwrite_data_flag = true
  ...
  def time_reset # Resets task objectives every monday
    ...
    if day_of_the_month < current_day
      ...
      @args.gtk.write_file(SAVE_DATA, "#{@reset_data}")
  ...
  def render_main_buttons
    ...
    @main_buttons||= ["NEW TASK"]

    if @back == true
      @main_buttons = ["BACK"]
    end

    if @new_task_object.length >= 4
      @main_buttons = ["BACK", "CREATE"]
    end
    
    @main_buttons.each do |buttons| 
      @main_button_text = buttons
    end


    @main_buttons.each_with_index do |item, index_x|
      main_buttons_x_positions =  (index_x * TILE_WIDTH) + HALF_SCREEN_X
  ...
  def new_task_grid()
    @new_task_options = [["TASK", nil], ["Permanent?", nil], ["Subtasks?", nil], ["Objective", nil],[ "Subobjectives", nil, nil ], []] #What's displayed on the current screen for the user to add its input on each division
  ...
  def permanent_and_temporary_tasks
  ...
  def new_task_user_input #Takes the parameters from user input to create the task, like the taask name, objectives, wether it has subobjectives or not, and if it has them, the su_tasks and subobjectives
    ...
    if @start_typing == true 
      ...
      if @args.inputs.keyboard.key_down.enter #Takes user input in the Task screen and has subobjective screen and takes user to the next screen 
        ...
        elsif new_task_has_subobjective? == "YES" # If the task has a sub objective takes input from the user in the sub objective screen
  ...
  def new_subtasks_and_subobjectives #Creates the subtasks and subobjective array for creating a new task object that has subtasks
    ...
    if @args.inputs.keyboard.key_down.enter && @current_screen > 4 #starts creating the array if the create tasks function goes to the subtasks screen
  ...
  def create_new_task() #Creates the new task object and adds them to the main task array
    ...
    task = @new_task_object[0]
    permanent = @new_task_object[1]
    has_subtask = @new_task_object[2]
    objective = @new_task_object[3].to_i
    subtask = []
    sub_objective = []
    
    # For reference @new_task_object = (task, permanent, has_subtask?, objective, subtask_and_subobjective)
    if @create == true
      if @new_task_object[2] != nil # Makes sure every element of the object is typed before creating the object
    
       if @required_screen == 3 # If the user choses the task doesn't have sub tasks or sub objectives it creates an object without them
          new_object = TasksAndSubTasks.new(task,[],[objective], has_subtask, permanent ).generated_task_and_subtask_hash
          puts new_object.permanent
       elsif @required_screen == 4
          @new_subtasks_and_subobjectives.each_pair do |key, value|
            subtask.append(key.upcase)
            sub_objective.append(value.to_i)
          end
          new_object = TasksAndSubTasks.new(task, subtask, sub_objective, has_subtask, permanent).generated_task_and_subtask_hash #If the user choses the task has sub tasks and sub objectives, it creates one
        end
        ...
        if permanent  == "YES"
          @reset_tasks.append(new_object) #If a task is permanent, saves it to be included every week in the initial tasks
          @args.gtk.write_file(MONDAY_RESET_DATA, "#{@reset_tasks}")
        end  
      end
    end
