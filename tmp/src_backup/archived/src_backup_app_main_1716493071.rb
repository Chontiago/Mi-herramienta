...
class TimeManageTool
  ...
  def new_task_behaviour()
    if @start_typing == true
      ...
      #@args.outputs.labels << {x: @grid_x, y: @new_task_grid_y, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{@word}"}
      @new_task_options.each do |task|
        task.each_with_index do |subtask, index|
        puts subtask
        end
      end
