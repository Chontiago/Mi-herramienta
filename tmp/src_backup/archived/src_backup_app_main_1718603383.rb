...
class TimeManageTool
  ...
  def array_buttons() #If task has no subtasks, adds or substracts from objective, if it has subtasks takes you to subtask menu to add them individually
    @create = false
    @clicked_task = nil
    @clicked_subtask ||= nil
      ...
      collumn.each_with_index do |item, index_x|
        ...
        if @args.inputs.mouse.click
          if mouse_click(@args.inputs.mouse.point, task_grid_traits) && @main_array[index_y][index_x] == '-'
            ...
            if @main_array[index_y][-1] == false
              @main_array[index_y][1] -= 1

            elsif @main_array == @task_array #Copies the title of the task, and compares it to the keys initia_tasks_array to see if it has subtasks, and if it has, the substract function changes the main array to the subtasks array
  ...
  def debug()
    @args.outputs.labels << {x: 1280 / 2, y: 100, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{@main_buttons[0]}"}
