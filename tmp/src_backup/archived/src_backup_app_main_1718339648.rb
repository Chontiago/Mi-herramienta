...
class TimeManageTool
  ...
  def new_task_user_input #Takes the parameters from user input to create the task, like the taask name, objectives, wether it has subobjectives or not, and if it has them, the su_tasks and subobjectives
    ...
    if @start_typing == true 
      ...
      if @args.inputs.keyboard.key_down.enter #Takes user input in the Task screen and has subobjective screen and takes user to the next screen 
        ...
        @new_task_object.append(new_task_has_subobjective?) #If the screen doesn't have a sub objective takes the input on the objective screen instead
        @user_input = []
        @start_typing = false
        @current_screen += 1
        end
  ...
  def new_subtasks_and_sub_objectives #Creates the subtasks and sub objective arrays for creating a new task object that has subtasks
    @new_subtasks ||= []
    @new_subobjectives = []
    @new_subtasks_and_sub_objectives = {}

    if @args.inputs.keyboard.key_down.enter && @current_screen > 3 #starts creating the arrays if the create tasks function goes to the subtasks screen
    ...
    @new_subtasks.each_slice(2) do |key, value|
      @new_subtasks_and_sub_objectives[key] = value
    end
      
    
    @args.outputs.labels << {x: 1280 / 2, y: 75, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{@new_subtasks_and_sub_objectives}"} 
  end
  ...
  def create_new_task()
    ...
    new_object = @new_task_object[0]
    if @create == true
      if @new_task_object[2] != nil # Makes sure every element of the object is typed before creating the object
    
       if @required_screen == 2 # Checks if the object has subtasks or subtasks and creates the object accordingly
          ...
          @new_subtasks_and_sub_objectives.each_pair do |key, value|
            subtask.append(key.upcase)
            sub_objective.append(value.to_i)
          end
          new_object = TasksAndSubTasks.new(@new_task_object[0].upcase, subtask, sub_objective, @new_task_object[1]).generated_task_and_subtask_hash
        end
  ...
  def debug()
    #@args.outputs.labels << {x: 1280 / 2, y: 100, anchor_x: 0.5, anchor_y: 0.5, r: 0, g: 0, b: 255, text:"#{user_input}"}
