...
class TimeManageTool
  ...
  def new_task_behaviour()
    @limit ||= nil #Variable that changes depending if new task created has subtasks or doesn't, and along with the counter variable takes to the appropiate screen when creating a new task

    if @limit != nil && @current_screen >=  @limit
      @current_screen = @limit
    end
