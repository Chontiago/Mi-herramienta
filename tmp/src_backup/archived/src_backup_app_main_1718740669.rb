...
class TimeManageTool
  ...
  def new_task_user_input #Takes the parameters from user input to create the task, like the taask name, objectives, wether it has subobjectives or not, and if it has them, the su_tasks and subobjectives
    ...
    @user_input.append(@args.inputs.text.first.upcase) #adds the characters that the user_input array to process them as full words
    ...
    if @create == true
      if @new_task_object[2] != nil # Makes sure every element of the object is typed before creating the object
       ...
       if @required_screen == 3 # If the user choses the task doesn't have sub tasks or sub objectives it creates an object without them
          new_object = TasksAndSubTasks.new(@new_task_object[0].upcase,[],[@new_task_object[3].to_i], @new_task_object[2], @new_task_object[1] ).generated_task_and_subtask_hash
          ...
          new_object = TasksAndSubTasks.new(@new_task_object[0].upcase, subtask, sub_objective, @new_task_object[1], false).generated_task_and_subtask_hash #If the user choses the task has sub tasks and sub objectives, it creates one
        end
