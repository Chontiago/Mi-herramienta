...
class TimeManageTool
  ...
  def render_new_task_button
    @new_task_buttons ||= [@main_button_text]
  ...
  def create_button
    ...
    if @object.length >= 3
      ...
      @new_task_buttons = ["BACK", @main_button_text]
      @new_task_buttons.each do |buttons|
        @new_task_buttons[1] = "CREATE"
    ...
    if @args.inputs.mouse.click
      if mouse_click(@args.inputs.mouse.point, @create_task_button ) && @new_task_buttons[1]== "CREATE"
        @create = true
        @new_task_buttons[1] = "NEW TASK"
        @main_button_text = "NEW TASK"
