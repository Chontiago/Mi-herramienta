    ...
    if @create == true
      if @new_task_object[2] != nil # Makes sure every element of the object is typed before creating the object
       ...
       if @required_screen == 2 # If the user choses the task doesn't have sub tasks or sub objectives it creates an object without them
          ...
          new_object = TasksAndSubTasks.new(@new_task_object[0].upcase, subtask, sub_objective, @new_task_object[1]).generated_task_and_subtask_hash #If the user choses the task has sub tasks and sub objectives, it creates one with them
        end
